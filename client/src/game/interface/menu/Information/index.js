...
export function Information(app, it) {
    it = Page(it);
    ...
    async function onPrev() {
        await Promise.all([carousel.prev(), animation(prevButton)]);
    }

    async function onNext() {
        await Promise.all([carousel.next(), animation(nextButton)]);
    }
}

function Carousel(it) {
    const pages = it.children
