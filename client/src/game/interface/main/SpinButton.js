...
export function SpinButton(app, it) {
    it = Button(it);
    ...
    function check(scores) {
        ...
        return [insufficientBalance, onAnyWin, onSingleWinOfAtLeast, ifCashIncreasesBy, ifCashDecreasesBy].some(isTrue);
    ...
    async function send(it) {
        if (insufficientBalance()) {
            const {value} = await app.alert.request({title: app.translate('common:helper.insufficientBalance')});
